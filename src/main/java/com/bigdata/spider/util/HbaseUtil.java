...
public class HbaseUtil {
	 ...
	 * HBASE 表名称
	 */
	public static final String TABLE_NAME = "sina";
	/**
	 * 列簇1
	 */
	public static final String COLUMNFAMILY_1 = "news";
	/**
	 * 列簇1中的列
	 */
	...
	public Page get(String tableName, String row) {
		...
		try {
			...
			if (raw.length >= 8) {
				page = new Page();
				page.setUrl(row);
				page.setCatgory(new String(raw[0].getValue()));
				page.setCommentCount(new String(raw[1].getValue()));
				page.setContent(new String(raw[2].getValue()));
				page.setKeywords(new String(raw[3].getValue()));
				page.setNewsFrom(new String(raw[4].getValue()));
				page.setTime(new String(raw[5].getValue()));
				page.setTitle(new String(raw[6].getValue()));
